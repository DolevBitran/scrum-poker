...
export const room: any = createModel<RootModel>()({
	...
	reducers: {
		...
		SET_SOCKET_MANAGER: (state: RoomState, payload: RoomState['socketManager']) => ({
			...state,
			socketManager: payload,
		}),
	},
	effects: (dispatch: Dispatch) => ({
		...
		async create(payload: CreateRoomProps, state: RootModel) {
			try {
				...
				this.SET_SOCKET_MANAGER(socketManager);
		...
		async vote(payload: VoteProps, state: RootModel) {
			try {
				...
				state.room.socketManager.vote(payload);
			} catch (err) {
				console.error(err);
				throw err;
			}
